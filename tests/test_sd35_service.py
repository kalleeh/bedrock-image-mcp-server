...
class TestBuildSD35Request:
    ...
    def test_text_to_image_without_negative_prompt(self):
        ...
        params = SD35TextToImageParams(
            prompt='A serene lake', aspect_ratio=AspectRatio.RATIO_1_1, seed=0
        )
    ...
    def test_image_to_image_without_negative_prompt(self):
        ...
        base64_image = create_test_image_base64()
        params = SD35ImageToImageParams(
            prompt='Add sunset colors', image=base64_image, strength=0.5, seed=0
        )
...
class TestGenerateTextToImage:
    ...
    async def test_generation_with_default_filename(
        self, mock_save_images, mock_invoke_bedrock, mock_bedrock_runtime_client
    ):
        ...
        # Create parameters
        params = SD35TextToImageParams(
            prompt='A peaceful forest', aspect_ratio=AspectRatio.RATIO_1_1, seed=0
        )

        # Call the function without filename
        result = await generate_text_to_image(
            params=params, bedrock_client=mock_bedrock_runtime_client
        )
    ...
    @pytest.mark.asyncio
    @patch('awslabs.bedrock_image_mcp_server.services.sd35_service.invoke_bedrock_model')
    async def test_api_error(self, mock_invoke_bedrock, mock_bedrock_runtime_client):
        ...
        # Create parameters
        params = SD35TextToImageParams(
            prompt='Test prompt', aspect_ratio=AspectRatio.RATIO_1_1, seed=0
        )

        # Verify exception is raised
        with pytest.raises(Exception, match='Bedrock API error'):
            await generate_text_to_image(params=params, bedrock_client=mock_bedrock_runtime_client)

    @pytest.mark.asyncio
    @patch('awslabs.bedrock_image_mcp_server.services.sd35_service.invoke_bedrock_model')
    async def test_no_images_returned(self, mock_invoke_bedrock, mock_bedrock_runtime_client):
...
class TestGenerateImageToImage:
    ...
    async def test_successful_generation_with_file_path(
        ...
        # Write a real image to disk so the service reads it from the filesystem
        img = Image.new('RGB', (128, 128), color='blue')
        image_path = os.path.join(temp_workspace_dir, 'input_image.png')
    ...
    async def test_generation_with_default_filename(
        ...
        # Create parameters
        params = SD35ImageToImageParams(
            prompt='Enhance details', image=base64_image, strength=0.3, seed=0
        )
    ...
    async def test_api_error(self, mock_invoke_bedrock, mock_bedrock_runtime_client):
        ...
        # Create parameters
        params = SD35ImageToImageParams(
            prompt='Test prompt', image=base64_image, strength=0.5, seed=0
        )
