...
"""Tests for the SD3.5 models module of the bedrock-image-mcp-server."""
...
class TestSD35TextToImageParams:
    ...
    def test_negative_prompt_length_validation(self):
        ...
        # Invalid: 10,001 characters
        with pytest.raises(ValidationError) as exc_info:
            SD35TextToImageParams(prompt='test', negative_prompt='b' * 10001)
...
class TestSD35ImageToImageParams:
    ...
    def test_valid_params(self, valid_base64_image):
        ...
        params = SD35ImageToImageParams(
            prompt='Transform this image', image=valid_base64_image, strength=0.7
        )
    ...
    def test_strength_bounds_minimum(self, valid_base64_image):
        ...
        # Valid: 0.0
        params = SD35ImageToImageParams(prompt='test', image=valid_base64_image, strength=0.0)
        assert params.strength == 0.0

        # Invalid: -0.1
        with pytest.raises(ValidationError) as exc_info:
            SD35ImageToImageParams(prompt='test', image=valid_base64_image, strength=-0.1)
    ...
    def test_strength_bounds_maximum(self, valid_base64_image):
        ...
        # Valid: 1.0
        params = SD35ImageToImageParams(prompt='test', image=valid_base64_image, strength=1.0)
        assert params.strength == 1.0

        # Invalid: 1.1
        with pytest.raises(ValidationError) as exc_info:
            SD35ImageToImageParams(prompt='test', image=valid_base64_image, strength=1.1)
    ...
    def test_image_dimension_validation_minimum(self, small_base64_image):
        ...
        with pytest.raises(ValidationError) as exc_info:
            SD35ImageToImageParams(prompt='test', image=small_base64_image, strength=0.7)
    ...
    def test_image_dimension_validation_valid(self, valid_base64_image):
        ...
        params = SD35ImageToImageParams(prompt='test', image=min_size_image, strength=0.7)
        assert params.image == min_size_image

    def test_invalid_base64_image(self):
        """Test that invalid base64 data is rejected."""
        with pytest.raises(ValidationError) as exc_info:
            SD35ImageToImageParams(prompt='test', image='not-valid-base64!!!', strength=0.7)
    ...
    def test_seed_bounds(self, valid_base64_image):
        ...
        # Valid: 0
        params = SD35ImageToImageParams(
            prompt='test', image=valid_base64_image, strength=0.7, seed=0
        )
        assert params.seed == 0

        # Valid: max
        params = SD35ImageToImageParams(
            prompt='test', image=valid_base64_image, strength=0.7, seed=4294967294
        )
        assert params.seed == 4294967294

        # Invalid: below minimum
        with pytest.raises(ValidationError):
            SD35ImageToImageParams(prompt='test', image=valid_base64_image, strength=0.7, seed=-1)

        # Invalid: above maximum
        with pytest.raises(ValidationError):
            SD35ImageToImageParams(
                prompt='test', image=valid_base64_image, strength=0.7, seed=4294967295
            )
